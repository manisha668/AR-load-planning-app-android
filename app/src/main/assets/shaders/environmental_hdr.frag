...
void main() {
  ...
  // Skip all lighting calculations if the estimation is not valid.
    ...
    vec3 base = texture(u_AlbedoTexture, texCoord).rgb;
    o_FragColor = vec4(base * u_Color.rgb, u_Color.a);
    return;
    }
  ...
  vec3 tintedColor = LinearToSrgb(radiance) * u_Color.rgb;
  o_FragColor = vec4(tintedColor, u_Color.a);
